...
import MyBuses from '../MyBuses/MyBuses';
...
export default function Adminpage() {
  ...
  //const [profile, setProfile] = React.useState(null); // Add profile state
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      >
        ...
        <Divider />
        <List>
          {['Dashboard', 'MyBuses', 'AllBuses', 'MyBookings'].map((text, index) => (
        ...
        <Divider />
        <List>
          {['Profile', 'Location', 'Logout'].map((text, index) => (
            <ListItem key={text} disablePadding>
              <ListItemButton onClick={() => handleselection(text)}>
                <ListItemIcon>
                  {index % 2 === 0 ? <InboxIcon /> : <MailIcon />}
